#!/usr/bin/env python3
"""Build a pinned, E-AC-3-only FFmpeg decoder as WebAssembly."""
...
EXPORTED_FUNCTIONS = (
    "_malloc",
    "_free",
    "_jellyfin_eac3_create",
    "_jellyfin_eac3_configure_packet",
    "_jellyfin_eac3_send_packet",
    "_jellyfin_eac3_receive_frame",
    "_jellyfin_eac3_get_plane",
    "_jellyfin_eac3_get_sample_format",
    "_jellyfin_eac3_get_sample_count",
    "_jellyfin_eac3_get_channel_count",
    "_jellyfin_eac3_get_channel_mask",
    "_jellyfin_eac3_get_sample_rate",
    "_jellyfin_eac3_get_pts",
    "_jellyfin_eac3_clear",
    "_jellyfin_eac3_destroy",
    "_jellyfin_eac3_library_version",
)
...
def build_ffmpeg(
    ...
    """Build only the pinned FFmpeg libraries needed by E-AC-3."""
    ...
    # FFmpeg enables the shared AC-3 decoder with its E-AC-3 decoder dependency
    require_ffmpeg_configuration(source_root, ("ac3", "eac3"))
    run_bash(
        bash_path,
        ["make", f"-j{jobs}"],
        source_root,
...
def link_decoder(
    ...
    """Link the focused FFmpeg libraries and bridge into one module."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    compiler = get_emscripten_tool(compiler_path, "emcc")
    if not bridge_path.is_file():
        raise RuntimeError("The pinned E-AC-3 bridge source is unavailable")
    command = [
        str(compiler),
        str(bridge_path),
...
def build_runtime_module(
    ...
    """Build one isolated runtime and return it with its upstream license."""
    ...
    output_path = build_root / "output" / "ffmpeg-eac3.mjs"
    link_decoder(
        bash_path,
        compiler_path,
        repository_root,
...
def parse_arguments() -> argparse.Namespace:
    """Parse focused decoder release-build arguments."""
...
def resolve_build_layout() -> tuple[Path, Path, Path, Path]:
    """Resolve source-tree or distributed relink-material paths."""
    source_repository_root = SCRIPT_DIRECTORY.parents[1]
    source_script_path = (
        source_repository_root / "scripts" / "webgpu" / "build_eac3_decoder.py"
    )
    if source_script_path.resolve() == Path(__file__).resolve():
        return (
            ...
            source_repository_root / "src" / "lib" / "ffmpeg-eac3",
            source_repository_root / "scripts" / "webgpu" / "eac3" / "artifacts",
        )
    return (
        SCRIPT_DIRECTORY,
        SCRIPT_DIRECTORY / "ffmpeg_eac3_bridge.c",
...
def main() -> int:
    """Build, verify, and record the focused E-AC-3 release artifact."""
    ...
    with tempfile.TemporaryDirectory(
        prefix="jellyfin-ffmpeg-eac3-release-"
        ...
        if arguments.verify_reproducible:
            ...
            if sha256_file(first_module) != sha256_file(second_module):
                raise RuntimeError("The isolated E-AC-3 rebuild was not reproducible")

        runtime_output_directory.mkdir(parents=True, exist_ok=True)
        copy_if_different(
            first_module,
            runtime_output_directory / "ffmpeg-eac3.mjs",
        )
        copy_if_different(
            upstream_license,
    ...
    runtime_module_path = runtime_output_directory / "ffmpeg-eac3.mjs"
    ...
    (artifact_output_directory / "REVISION").write_text(
        ...
        "FFmpeg source archive: ../../truehd/artifacts/ffmpeg-source.tar.gz\n"
    ...
    print(
        f"Built FFmpeg {FFMPEG_COMMIT} E-AC-3 decoder with "
        f"Emscripten {EMSCRIPTEN_VERSION}"
    )
    return 0
